...
{
    public class SelfAndOneOpponentCounterModule : CounterModuleBase
    {
        ...
        public override void PerformKCyclicNeighborQUpdate(QTableBase qtable)
        {
            ...
            // opr for opponent row
            for (int opr = 0; opr < m_rows; ++opr)
            {
                // opc for opponent col
                for (int opc = 0; opc < m_cols; ++opc)
                {
                    ...
                    for (int boi = 0; boi < m_ballOwnerStatesCount; ++boi)
                    {
                        for (int r = 0; r < m_rows; ++r)
                        {
                            ...
                            {
                                double support = GetSupport(r, c, opr, opc, boi);
                                ...
                                if (support < Params.DM.MinSupport)
                                {
                                    ...
                                    {
                                        PerformAveraging(qtable, opr, opc, boi, r, c);
                                    }
                                    else if (Params.DM.Method == Params.DM.MethodTypes.TopQ)
                                    {
                                        PerformTopQ(qtable, opr, opc, boi, r, c);
                                    }
                                    else if(Params.DM.Method == Params.DM.MethodTypes.Voting)
                                    {
                                        PerformVoting(qtable, opr, opc, boi, r, c);
                                    }
                                }
                            }
                        }
        ...
        private double GetActionConfidence(int r, int c, int opr, int opc, int boi, int ai)
        {
            double support = GetSupport(r, c, opr, opc, boi);
            return ((double)m_counterTable[r, c, opr, opc, boi, ai]) / support;
        }

        private double GetSupport(int r, int c, int opr, int opc, int boi)
        {
            double sum = 0.0;

            for (int i = 0; i < m_actionsCount; ++i)
            {
                sum += m_counterTable[r, c, opr, opc, boi, i];
            }
