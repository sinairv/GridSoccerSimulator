...
namespace SharpNeatLib
{
	...
	public class GenomeDecoder
	{
		#region Decode To ConcurrentNetwork
		 ...
		 public INetwork DecodeToConcurrentNetwork(NeatGenome.NeatGenome g, IActivationFunction activationFn)
		{
		...
		/// </summary>
		// FastConnectionComparer fastConnectionComparer = new FastConnectionComparer();		
		private FloatFastConnection[] fastConnectionArray;
        private IActivationFunction[] activationFunctionArray;

		public FloatFastConcurrentNetwork DecodeToFloatFastConcurrentNetwork(NeatGenome.NeatGenome g, IActivationFunction activationFn)
		{			
		...
		public FastConcurrentMultiplicativeNetwork DecodeToFastConcurrentMultiplicativeNetwork(NeatGenome.NeatGenome g, IActivationFunction activationFn)
		{
		...
		/// </summary>
		// FastConnectionComparer fastConnectionComparer = new FastConnectionComparer();		
		private IntegerFastConnection[] intFastConnectionArray;

		public IntegerFastConcurrentNetwork DecodeToIntegerFastConcurrentNetwork(NeatGenome.NeatGenome g)
		{			
			int outputNeuronCount = g.OutputNeuronCount;
			int neuronGeneCount = g.NeuronGeneList.Count;
		...
		#region Decode To NetworkModel
		 ...
		 public NetworkModel DecodeToNetworkModel(ConcurrentNetwork network)
		{
		 ...
		 public NetworkModel DecodeToNetworkModel(NeatGenome.NeatGenome g)
		{
			ModelNeuronList masterNeuronList = new ModelNeuronList();
		...
		private  int CompareKeys(ref FloatFastConnection a, ref FloatFastConnection b)
		{
		...
		private  void QuickSortFastConnections(int left, int right) 
		{
			do 
			{
				int i = left;
		...
		private  int CompareKeys(ref IntegerFastConnection a, ref IntegerFastConnection b)
		{
			int diff = a.sourceNeuronIdx - b.sourceNeuronIdx;
			if(diff==0)
			{
		...
		/// <param name="left"></param>
		/// <param name="right"></param>
		private  void QuickSortIntFastConnections(int left, int right) 
		{
			do 
			{
				int i = left;
