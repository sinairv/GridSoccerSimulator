...
namespace NeuralNetwork.Learning
{
	...
	public class BackPropagationLearning : ISupervisedLearning
	{
        ...
        /// for each vector provided in the <paramref name="input"/> array.</para>
        /// </remarks>
		...
		private double CalculateError( double[] desiredOutput )
		{
			...
			for ( int i = 0, n = layer.NeuronsCount; i < n; i++ )
			{
				...
				errors[i] = e * neuron.ActivationFunction.DerivativeUsingValue( output );

                Debug.Assert(!Double.IsNaN(errors[i]) && !Double.IsInfinity(errors[i]));
			...
			for ( int j = layersCount - 2; j >= 0; j-- )
			{
				...
				for ( int i = 0, n = layer.NeuronsCount; i < n; i++ )
				{
					...
					errors[i] = sum * neuron.ActivationFunction.DerivativeUsingValue( neuron.Output );
                    Debug.Assert(!Double.IsNaN(errors[i]) && !Double.IsInfinity(errors[i]));
				}
			}
		...
		private void CalculateUpdates( double[] input )
		{
			...
			for ( int i = 0, n = layer.NeuronsCount; i < n; i++ )
			{
                ...
                Debug.Assert(!Double.IsNaN(layerThresholdUpdates[i]) && !Double.IsInfinity(layerThresholdUpdates[i]));
			}
			...
			for ( int i = 0, n = network.LayersCount; i < n; i++ )
			{
				...
				for ( int j = 0, m = layer.NeuronsCount; j < m; j++ )
				{
					...
					for ( int k = 0, s = neuron.InputsCount; k < s; k++ )
					{
                        ...
                        Debug.Assert(!Double.IsInfinity(neuron[k]) && !Double.IsNaN(neuron[k]));
					}
					// update treshold
					neuron.Threshold += layerThresholdUpdates[j];
                    Debug.Assert(!Double.IsNaN(neuron.Threshold) && !Double.IsInfinity(neuron.Threshold));
                }
			}
		}
	}
