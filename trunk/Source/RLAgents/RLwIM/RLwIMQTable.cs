...
namespace RLwIM
{
    class RLwIMQTable : QTableBase
    {
        ...
        public RLwIMQTable(int rows, int cols, int teammatesCount, int oppsCount, int myUnum)
        {
            m_myUnum = myUnum;
            m_teammatesCount = teammatesCount;
            m_opponentsCount = oppsCount;

            // TODO: instantiate the modules and add them to the m_modules list
            AddModule(new TripleModulewIM(rows, cols, teammatesCount, oppsCount, myUnum, 0, 0));
            AddModule(new TripleModulewIM(rows, cols, teammatesCount, oppsCount, myUnum, 0, 1));
        }
